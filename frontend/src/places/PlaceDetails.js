...
function PlaceDetails() {
	...
	useEffect(() => {
		const fetchData = async () => {
			const response = await fetch(`http://192.168.0.29:5000/places/${placeId}`)
	...
	async function deletePlace() {
		await fetch(`http://192.168.0.29:5000/places/${place.placeId}`, {
			method: 'DELETE'
		})
		history.push('/places')
	}
	...
	async function deleteComment(deletedComment) {
		await fetch(`http://192.168.0.29:5000/places/${place.placeId}/comments/${deletedComment.commentId}`, {
			method: 'DELETE'
		})
